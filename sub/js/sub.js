...
const initSubCommon = () => {
  const initSubTabs = () => {
    ...
    const tabGroups = [
      {
        label: "병원소개 메뉴",
        tabs: [
          { href: "./greeting.html", label: "인사말" },
          { href: "./doctor.html", label: "의료진소개" },
          { href: "./guide.html", label: "진료안내" },
          { href: "./tour.html", label: "둘러보기" },
      ...
      },
      {
        label: "조루센터 메뉴",
        tabs: [
          { href: "./premature.html", label: "조루증이란?" },
          { href: "./premature-dorsal.html", label: "배부신경차단술" },
          { href: "./premature-glans-nerve.html", label: "귀두미세신경차단술" },
          { href: "./premature-frenulum.html", label: "소대신경차단술" },
          { href: "./premature-combo.html", label: "조루복합수술" },
          { href: "./premature-dermis.html", label: "진피이식조루수술" },
        ],
      },
      {
        label: "발기부전 메뉴",
        tabs: [
          { href: "./erectile.html", label: "발기부전이란?" },
          { href: "./ed-medication.html", label: "약물치료" },
          { href: "./ed-hormone.html", label: "호르몬요법" },
          { href: "./ed-trimix.html", label: "트리믹스주사요법" },
          { href: "./ed-malleable.html", label: "굴곡형보형물삽입술" },
          { href: "./ed-inflatable.html", label: "팽창형보형물삽입술" },
        ],
      },
      {
        label: "정관·포경 메뉴",
        tabs: [
          { href: "./vasectomy.html", label: "무도정관수술" },
          { href: "./circumcision-laser.html", label: "레이져 포경수술" },
          { href: "./circumcision-sleeve.html", label: "슬리브 포경수술" },
          { href: "./circumcision-roll.html", label: "마는포경수술" },
          { href: "./circumcision-sunflower.html", label: "해바라기포경" },
        ],
      },
      {
        label: "특수수술 메뉴",
        tabs: [
          { href: "./vaseline-removal.html", label: "바세린제거수술" },
          { href: "./curvature.html", label: "음경만곡증수술" },
          { href: "./webbed-scrotum.html", label: "음낭갈퀴제거수술" },
          { href: "./testis-implant.html", label: "인공고환삽입술" },
          { href: "./pubic-augmentation.html", label: "치골융기술" },
        ],
      },
      {
        label: "외래검진센터 메뉴",
        tabs: [
          { href: "./prostate.html", label: "전립선 질환" },
          { href: "./urinary-stone.html", label: "요로결석" },
          { href: "./andropause.html", label: "남성갱년기" },
          { href: "./semen-test.html", label: "정액검사" },
          { href: "./wedding-check.html", label: "웨딩검진" },
        ],
      },
    ];
    const currentFile = window.location.pathname.split("/").pop();
    const activeGroup = tabGroups.find((group) =>
      group.tabs.some((tab) => tab.href.replace("./", "") === currentFile)
    );
    const activeTab = activeGroup?.tabs.find((tab) => tab.href.replace("./", "") === currentFile);
    ...
    nav.setAttribute("aria-label", activeGroup.label);
  ...
  const initMobileMainNav = () => {
    ...
    const navItems = Array.from(nav.querySelector("ul")?.children || []).filter(
      (item) => item.querySelector(".main-nav__depth1") && item.querySelector(".mega-menu")
    );

    const setSubmenuOpen = (item, isOpen) => {
      const button = item.querySelector(".main-nav__depth1");
    ...
    const closeSubmenus = () => {
      navItems.forEach((item) => setSubmenuOpen(item, false));
    };
    ...
    navItems.forEach((item, index) => {
      ...
      if (!submenu.id) {
        submenu.id = `main-nav-submenu-${index + 1}`;
      }
